...
The backend selects Fortran wrappers, imports the compiled extension `twoelint_f90`, and contains Python fallback paths when the extension is unavailable. All six Fortran source files are included; a compatible compiled extension must still be built for Fortran execution. Compilation and numerical Python/Fortran agreement have not been validated.

The driver's misspelled `soa_cao_trasnform` import points to the supplied `final_codes.sao_cao_transform` module. `run_ct_with_logging.py` is generated by the launcher. Separate `calc_AAAA.py` and `calc_BBBB.py` modules are not needed by the supplied imports.
...
`psutil` is a Python library for reading process and system resource information ([official documentation](https://psutil.io/)). In this project, `psutil.cpu_count()` supplies CPU counts and `psutil.virtual_memory().available` supplies available RAM for worker and buffer sizing. It must be installed in the same Python environment used for calculations.
...
To add these checks to an existing copy of the repository, copy the `scripts/` and `tests/` folders and this README from the updated package into the project root. Run the commands above, then commit those files. These checks run when invoked; no automatic GitHub workflow has been added.
...
| Real module imports | Partially passed | The three mixed Python kernels, one-electron/AO-transform modules, MO converter, and other checked modules loaded. Missing `psutil` in the review environment blocked the three integral processors and main driver. |
...
The missing `psutil` above refers to the review environment; it is already listed in `requirements.txt`. It does not indicate that this package is absent from your calculation computer. The mock test definitions are included under `tests/`, and dummy executables/modules are generated only inside temporary test folders.

Remaining scientific/integration questions include the two processor schemas, left/right AO-vector convention differences between helper functions and the processor, coefficient selection, occupation parsing, and compiled-backend compatibility. The provided geometry input also contains an apparent stray `S` on a reference-table number (`-0.0001051477S`); it was preserved because reference-table cleanup is outside these setup changes.

For reproducible publication, add validated software versions, a complete small example with expected results and tolerances, numerical Python/Fortran regression checks, and an owner-selected license. Do not assume a license from the presence of source files.

The latest `twoelint_calc_fortran.py` is included byte-for-byte from the upload. It retains its own forced autotune setting and duplicate shared-vector preparation, unlike the earlier corrected alternative processors. These are implementation issues to review, not missing files. No additional runtime or numerical fixes were applied to this final upload.
