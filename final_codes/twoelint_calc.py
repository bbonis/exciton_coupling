...
def _get_twoelint_f90():
    ...
    """
    global _twoelint_f90, _HAS_TWOELINT_F90
    if _twoelint_f90 is None:
        try:
...
def calc_BAAA_f90_zero_copy(
    ...
    """
    if (not _HAS_TWOELINT_F90) or (SC_A_BA_ia is None):
        return calc_BAAA(
            twoelint_block,
    ...
    out = _twoelint_f90.twoelint_baaa_mod.baaa_accum_all(
...
def calc_BBBB_f90_zero_copy(
    ...
    return _twoelint_f90.twoelint_bbbb_mod.bbbb_accum_all(
        pos1=pos1, pos2=pos2, pos3=pos3, pos4=pos4,
        a=a_i16, b=b_i16, c=c_i16, d=d_i16, v=v_f64,
...
def calc_AAAA_f90_zero_copy(
    ...
    return _twoelint_f90.twoelint_aaaa_mod.aaaa_accum_all(
        pos1=pos1, pos2=pos2, pos3=pos3, pos4=pos4,
        a=a_i16, b=b_i16, c=c_i16, d=d_i16, v=v_f64,
        threshold=int(threshold),
...
def calc_BBBA_f90_zero_copy(
    ...
    _f90 = globals().get("_twoelint_f90", None)

    py_fallback = globals().get("calc_BBBA")
    if (not _HAS) or (_f90 is None) or (SC_A_BA_ia is None) or (py_fallback is None):
        return py_fallback(
            twoelint_block,
            SC_A_BA_ia,
            CS_A_AB_ia,
...
def calc_BBAA_f90_zero_copy(
    ...
    out, err_flag = _twoelint_f90.twoelint_bbaa_mod.bbaa_accum_all(
        pos2=pos2,
        pos3=pos3,
        pos4=pos4,
        a=a,
...
def process_chunk(
    ...
    with attached_vectors(vec_handles) as vectors:
        required_overlap_keys = ('SC_A_BA_ia', 'CS_A_AB_ia', 'SC_B_AB_ia', 'CS_B_BA_ia', 'CS_EA_A_a', 'SC_IP_B_i', 'SC_IP_A_i', 'CS_EA_B_a', 'SC_A_BA_pa', 'CS_A_AB_iq', 'SC_B_AB_pa', 'CS_B_BA_iq', 'CS_EA_A_q', 'SC_IP_B_p', 'SC_IP_A_p', 'CS_EA_B_q', 'SCS_A_BB_iq', 'SCS_A_BB_pa', 'SCS_B_AA_pa')
        missing = [key for key in required_overlap_keys if key not in vectors]
        if missing:
            raise ValueError("Missing required AO overlap vectors: " + ", ".join(missing))
        # Resolve array views
        SC_A_BA_ia = vectors['SC_A_BA_ia']
        CS_A_AB_ia = vectors['CS_A_AB_ia']
        SC_B_AB_ia = vectors['SC_B_AB_ia']
        CS_B_BA_ia = vectors['CS_B_BA_ia']
        CS_EA_A_a  = vectors['CS_EA_A_a'] 
        SC_IP_B_i  = vectors['SC_IP_B_i'] 
        SC_IP_A_i  = vectors['SC_IP_A_i'] 
        CS_EA_B_a  = vectors['CS_EA_B_a'] 
        SC_A_BA_pa = vectors['SC_A_BA_pa']
        CS_A_AB_iq = vectors['CS_A_AB_iq']
        SC_B_AB_pa = vectors['SC_B_AB_pa']
        CS_B_BA_iq = vectors['CS_B_BA_iq']
        CS_EA_A_q  = vectors['CS_EA_A_q'] 
        SC_IP_B_p  = vectors['SC_IP_B_p'] 
        SC_IP_A_p  = vectors['SC_IP_A_p'] 
        CS_EA_B_q  = vectors['CS_EA_B_q'] 
        SCS_A_BB_iq = vectors['SCS_A_BB_iq'] 
        SCS_A_BB_pa = vectors['SCS_A_BB_pa'] 
        SCS_B_AA_pa = vectors['SCS_B_AA_pa'] 
